 ...
 */
class Shops_Model_ShopBase extends Shops_Model_Base_ShopBase
{
    /**
     * Путь к изображению относительно public
     */
    const PUBLIC_UPLOAD_PATH = '/upload/images/shops';
     ...
     * Путь к изображению относительно public
     */
    const SUFFIX_THUMBNAIL_GREYSCALE_IMAGE = '-thumb-greyscale';
    ...
    public function createImageThumbnail(ZFEngine_Image_Adapter_Abstract $image)
    {
        ...
        $config['width'] = 60;
        $config['height'] = 40;
        $image->resize($config['width'], $config['height'], true, ZFEngine_Image_Adapter_Abstract::HEIGHT)
              ->saveAs($imageThumbFileName, 95);

        $image = ZFEngine_Image::factory($imageThumbFileName);
        $imageThumbGreyscaleFileName = $this->getImageAbsoluteUploadPath()
                              . DIRECTORY_SEPARATOR
                              . ZFEngine_File::addSuffixToFileName($this->image_filename, self::SUFFIX_THUMBNAIL_GREYSCALE_IMAGE);
        $image->filterGrayscale()
              ->saveAs($imageThumbGreyscaleFileName, 95);
     ...
     * @return string
     */
    public function getImageThumbnailGreyscaleUrl()
    {
        if (strlen($this->image_filename)) {
            return self::PUBLIC_UPLOAD_PATH
                . '/'
    ...
    public function unlinkImages()
    {
        ...
        if (file_exists($this->getImageFullPath('thumbnail-greyscale'))) {
            unlink($this->getImageFullPath('thumbnail-greyscale'));
        }
    }
